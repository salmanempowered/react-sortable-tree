...
export const getDescendantCount = ({ node, ignoreCollapsed = true, }) => {
...
export const getVisibleNodeCount = ({ treeData }) => {
    const traverse = (node) => {
        if (!node.children ||
            node.expanded !== true ||
...
export const getVisibleNodeInfoAtIndex = ({ treeData, index: targetIndex, getNodeKey, }) => {
...
export const walk = ({ treeData, getNodeKey, callback, ignoreCollapsed = true, }) => {
    if (!treeData || treeData.length === 0) {
        return;
    }
    walkDescendants({
...
export const map = ({ treeData, getNodeKey, callback, ignoreCollapsed = true, }) => {
    if (!treeData || treeData.length === 0) {
        return [];
    }
...
export const toggleExpandedForAll = ({ treeData, expanded = true, }) => {
    return map({
        treeData,
...
export const changeNodeAtPath = ({ treeData, path, newNode, getNodeKey, ignoreCollapsed = true, }) => {
...
export const removeNodeAtPath = ({ treeData, path, getNodeKey, ignoreCollapsed = true, }) => {
...
export const removeNode = ({ treeData, path, getNodeKey, ignoreCollapsed = true, }) => {
    let removedNode;
...
export const getNodeAtPath = ({ treeData, path, getNodeKey, ignoreCollapsed = true, }) => {
    let foundNodeInfo;
    try {
        changeNodeAtPath({
            treeData,
...
export const addNodeUnderParent = ({ treeData, newNode, parentKey = undefined, getNodeKey, ignoreCollapsed = true, expandParent = false, addAsFirstChild = false, }) => {
...
export const insertNode = ({ treeData, depth: targetDepth, minimumTreeIndex, newNode, getNodeKey, ignoreCollapsed = true, expandParent = false, }) => {
...
export const getFlatDataFromTree = ({ treeData, getNodeKey, ignoreCollapsed = true, }) => {
    if (!treeData || treeData.length === 0) {
        return [];
    }
    const flattened = [];
...
export const getTreeFromFlatData = ({ flatData, getKey = (node) => node.id, getParentKey = (node) => node.parentId, rootKey = '0', }) => {
    if (!flatData) {
        return [];
    }
...
export const getDepth = (node, depth = 0) => {
    if (!node.children) {
        return depth;
    }
    if (typeof node.children === 'function') {
...
export const find = ({ getNodeKey, treeData, searchQuery, searchMethod, searchFocusOffset, expandAllMatchPaths = false, expandFocusMatchPaths = true, }) => {
    let matchCount = 0;
